        ...
        let evm_result = EvmResult {
            result: match self {
                ExecutionResult::Success { reason, gas_used, gas_refunded, logs, output } => {
                    Some(ResultType::Success(Success {
                        reason: match reason {
                            ...
                            revm::primitives::SuccessReason::Return => {
                                SuccessReasonEnum::Return.into()
                            }
                            ...
                            .map(|log| Log {
                                ...
                                data: Some(LogData {
                                    topics: log
                                        ...
                                        .topics()
                                        .iter()
                                        .map(|topic| Topic { value: topic.to_vec() })
                                        .collect(),
                ...
                ExecutionResult::Revert { gas_used, output } => {
                    Some(ResultType::Revert(Revert { gas_used, output: output.to_vec() }))
                }
